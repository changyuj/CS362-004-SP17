...
#define HAND_SIZE 27
 ...
 int checkSmithy(int thisPlayer, struct gameState *testG, int handPos)
 {
	...
	int initial_hand, initial_played_card, test_num = 0;
	
	initial_hand = testG->handCount[thisPlayer];
	initial_played_card = testG->playedCardCount;
...
#if (NOISY_TEST == 1)
    printf("Testing player %d played card count... played card cound should increase by 1\n", thisPlayer+1);
	printf("\nplayed card count: %d\tExpected: %d\n",G2.playedCardCount, initial_played_card+1);
#endif
	test_num += assertEqual(initial_played_card+1, G2.playedCardCount);
...
#if (NOISY_TEST == 1)
            printf("\nTest Number %d\tplayer: %d\tdeckCount: %d\thandCount: %d\tplayedCardCount: %d\n\n",i+1, n+1, G.deckCount[n],G.handCount[n],G.playedCardCount);
#endif		
